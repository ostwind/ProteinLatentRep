...
class SimilarityRegression(nn.Module):
    def __init__(self, emb_dim, rna_dim):
        super(SimilarityRegression, self).__init__()
        ...
        self.lin3 = nn.Linear(emb_dim, emb_dim, bias = False)
    
    def init_weights(self):
        for x in self.lin3.parameters():
    ...
    def forward(self, embs, rna_samples):
        x = self.lin3(embs)
        x = torch.matmul(x, rna_samples) # yy^T for training data
        return x
