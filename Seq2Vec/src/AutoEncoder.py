...
class RRM_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        rrm = torch.from_numpy(rrm).contiguous()
...
class VAE(nn.Module):
	...
	def encode(self, x):
		...
		h2 = self.relu(self.lin2(x)) #900
		return self.lin31(h1), self.lin32(h2) #100, 100
...
def loss_function(recon_x, x, mu, logvar):
	...
	BCE = F.F.binary_cross_entropy(recon_x, x)
...
def train(epoch):
	...
	for batch_idx, data in enumerate(train_loader):
		data = Variable(data.view(-1, 3608))
		if args.cuda:
			data = data.cuda()
		optimzer.zero_grad()
