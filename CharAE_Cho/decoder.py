...
class AttnDecoderRNN(nn.Module):
    def __init__(self, hidden_size, seq_len, n_layers=1, dropout_p=0.5,):
        super(AttnDecoderRNN, self).__init__()
        self.hidden_size = hidden_size
        self.n_layers = n_layers
        
        self.attn = nn.Linear(3*hidden_size, seq_len) # due to bi-directional gru
        self.attn_combine = nn.Linear(3*hidden_size, hidden_size)
        #self.dropout = nn.Dropout(dropout_p)
        
        self.gru = nn.GRU(hidden_size, 2*hidden_size) # decoders cant be bi-directional
        #TODO changed last linear layer from size 22 
        self.out = nn.Linear(2*hidden_size, 23)

    def forward(self, embedded, hidden, encoder_outputs, rnn_emit_len):
        ...
        #print(embedded.data.shape, attn_applied.data.shape, output.data.shape)
